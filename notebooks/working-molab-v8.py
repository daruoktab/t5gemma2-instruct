...
#
# =====================================================================
# T5Gemma-2 JOINT MULTIMODAL PIPELINE (v8 — Research SOTA & Alignment)
# =====================================================================
#   Phase 0.5 : DeVec SVD Subspace Filtering + Layer-Wise Ramp-Up Steering
...
#   steered/  -> checkpoint hasil Phase 0.5 (DeVec SVD Purified)
...
def _():
    ...
    OUTPUT_DIR = "results/t5gemma2_joint_v8"   # working dir lokal (checkpoints, logs, merge)
    ...
    # =====================================================================
    ENABLE_DEVEC_STEERING = True  # SVD Subspace Filtering to isolate dominant signal
    DEVEC_SVD_TAU = 0.85          # Subspace energy threshold
    ENABLE_LATA_ALIGNMENT = True  # Layer-aware task-arithmetic alignment modulation (arXiv:2502.20186)
    ...
    # =====================================================================
    # 1D. DATA & MIXING & MTO PREFIXING (Joint Co-Training) (arXiv:2606.24841)
    # =====================================================================
    ...
    # =====================================================================
    # 1F. ORPO + TLPO HYPERPARAMS (Phase 2 - Joint) (arXiv:2604.26553)
    # =====================================================================
    ORPO_BETA = 0.1
    TLPO_LAMBDA = 0.05            # Weight of Token-Level Policy penalty
    TLPO_CLIP_EPS = 0.2           # PPO style ratio clipping for policy update
    ENABLE_TLPO_LOSS = True       # Heuristic suppress-ids proxying (bukan paper TLPO 2604.26553)

    ENABLE_FLAW_AWARE_LOSS = True # Option 3: bobot loss ORPO dari kolom `flaw` (+ emphasis token repetitive)
    FLAW_AWARE_LAMBDA = 0.1       # bobot aux token-level emphasis (repetitive n-gram pada rejected)
    ...
    # =====================================================================
    # 1G. OPTIMIZER: OrScale-LM + GrokFast + AdEMAMix (arXiv:2605.07815)
    # =====================================================================
    # "grokorscale"   = GrokFast filter + OrScaleLM (2D, Newton-Schulz + Trust Ratio Scaling) + AdEMAMix (1D)
    # "grokmuonadema" = GrokFast + Muon (2D) + AdEMAMix (1D)
    # "grokademamix"  = GrokFast + AdEMAMix murni
    # "paged_adamw_8bit" = bawaan HF/Unsloth fallback
    OPTIMIZER_TYPE = "grokorscale"
    ...
    ADEMA_BETA3 = 0.9999
    # OrScaleLM / Muon
    ORSCALE_MOMENTUM = 0.95
    ORSCALE_NS_STEPS = 5
    ORSCALE_NESTEROV = True
    ORSCALE_MAX_GRAD_NORM = 1.0       # Gradient clip threshold
    ORSCALE_R_MIN = 0.1              # Trust ratio lower clamping boundary
    ORSCALE_R_MAX = 5.0              # Trust ratio upper clamping boundary

    # LR Scale multipliers for OrScale / Muon
    SFT_ORSCALE_SCALE = 20.0         # mis. decoder SFT: 5e-6 * 0.2 * 20 = 2e-5
    ORPO_ORSCALE_SCALE = 5.0         # mis. decoder ORPO: 5e-6 * 1.0 * 5 = 2.5e-5

    # Routing komponen multi_modal_projector (2D bobotnya):
    #   "orscale" = ikut aturan 2D -> cabang OrScaleLM
    #   "adema"   = paksa ke cabang AdEMAMix (konservatif)
    PROJECTOR_BRANCH = "orscale"
...
def _(mo):
    mo.md(r"""
    ✅ **STATUS: V8 RESEARCH SOTA PIPELINE READY.**

    # 🔗 T5Gemma-2 v8 — Multimodal Research SOTA & Alignment Pipeline

    ---

    ### 🗺️ Alur Pipeline Multimodal V8 (Sequential DAG)

    | Tahap | Nama Phase | Landasan Riset / Deskripsi Komponen | Output Checkpoint |
    |---|---|---|---|
    | **0.5** | **DeVec SVD Task Vector Steering** | *TV_DECOMP* ([2512.22511](https://arxiv.org/abs/2512.22511)): SVD subspace filtering ($\tau=0.85$) + Layer-Wise Ramp-Up FFN & Norm. Attn Q/K/V di-preserve ($0.0$). | `steered/` |
    | **1.5** | **Vision Grafting** | *Gemma 3 TR* ([2503.19786](https://arxiv.org/abs/2503.19786)): Transplantasi `SigLIP 400M` + `multi_modal_projector` dari Gemma 3 4B IT + Pure BF16 Cast Guard. | `cangkok/` (Base Model) |
    | **1.0** | **JOINT SFT + MTO Prefixing** | *MTO* ([2606.24841](https://arxiv.org/abs/2606.24841)) + *OrScale-LM* ([2605.07815](https://arxiv.org/abs/2605.07815)): Co-training Vision + Text SFT, Assistant Target Prefix (`<unused1>`..`<unused6>`), OrScaleLM Trust Ratio Optimizer. | `joint/sft/` |
    | **2.0** | **JOINT ORPO + TLPO Regularization** | *TLPO* ([2604.26553](https://arxiv.org/abs/2604.26553)): Multi-modal preference tuning + Token-Level Policy penalty against Indonesian language confusion ($\lambda=0.05$). | `joint/orpo/` |
    | **Final** | **Unified Single Merge** | Single-stage LoRA merge ke `final/merged_bf16` + `final/quantized_4bit` via Unsloth safe saving patch. | `final/merged_bf16`, `final/quantized_4bit` |

    > ⚙️ **Semua konfigurasi tweakable ada di cell CONTROL CENTER di atas.**
    """)
    return
...
def _(SYSTEM_PROMPT, re):
    ...
    # ---- MTO TASK PREFIX FORMATTER (arXiv:2606.24841) ----
    def format_mto_target(target_text: str, task_category: str = "general_chat") -> str:
        """
        Suntikkan Task-Specific Prefix Token di awal target Assistant:
          <unused1> : Summarization
          <unused2> : Translation
          <unused3> : NER
          <unused4> : QA (IndoQA)
          <unused5> : Paraphrase
          <unused6> : General Chat / Vision Reasoning
        """
        prefix_mapping = {
            "summarize": "<unused1>",
            "translate": "<unused2>",
    ...
    # ---- MTO OBJECTIVE MAPPING & ENCODER TEMPLATE (arXiv:2606.24841) ----
    MTO_OBJECTIVES = {
        "summarize": "map_phrasal",
        "translate": "map_phrasal",
        "ner": "mask_filling",
        "qa": "mask_filling",
        "paraphrase": "map_phrasal",
        "general_chat": "map_phrasal",
        "vision": "map_phrasal",
    }

    def format_mto_encoder_input(raw_input: str, task_category: str = "general_chat") -> str:
        """
        Terapkan template objective-aware pada encoder input.
        - mask_filling (QA/NER): sisipkan sentinel <extra_id_0> di posisi jawaban.
        - map_phrasal (chat/summarize/translate/paraphrase): pertahankan format chat.
        """
        objective = MTO_OBJECTIVES.get(task_category.lower(), "map_phrasal")
        encoder = format_encoder_from_raw(raw_input)
        if objective == "mask_filling":
            _turn_marker = "<start_of_turn>model\n"
            if encoder.endswith(_turn_marker):
                encoder = encoder[: -len(_turn_marker)] + " <extra_id_0>\n" + _turn_marker
        return encoder

    return format_encoder_from_raw, format_mto_encoder_input, format_mto_target
...
def _(torch):
    ...
    # ---------- OrScale / Muon primitive: 5-step Quintic Newton-Schulz ----------
    def zeropower_via_newtonschulz5(
        ...
        apply_shape_scale=True  -> Muon: polar factor dikali max(1, sqrt(m/n)).
        apply_shape_scale=False -> polar factor murni (untuk OrScale-LM; Moonlight
                                   s_l = 0.2*sqrt(max(m,n)) yang handle shape scaling).
        """
    ...
    # ---------- GrokOrScale: OrScale-LM (2D) + AdEMAMix (1D) (arXiv:2605.07815) ----------
    class GrokOrScale(torch.optim.Optimizer):
        """
        OrScale-LM (Orthogonalised Optimization with Layer-Wise Trust Ratio Scaling) + GrokFast + AdEMAMix:
        - Cabang 2D: Newton-Schulz polar update + Moonlight shape factor + Frobenius Trust Ratio Scaling.
        - Cabang 1D: AdEMAMix dual-EMA.
        - GrokFast gradient amplification & MuonClip gradient threshold.
        """
        def __init__(
            self,
            params,
            ...
            for group in self.param_groups:
                ...
                for p in group["params"]:
                    ...
                    if _use_orscale:
                        ...
                        # Polar factor Q (murni; Moonlight s_l yang handle shape scaling)
                        Q = zeropower_via_newtonschulz5(g_update, steps=ns_steps, apply_shape_scale=False)

                        # Moonlight shape factor s_l = 0.2 * sqrt(max(m, n))
    ...
    return GrokAdEMAMix, GrokMuonAdEMA, GrokOrScale


@app.cell
def _(GrokAdEMAMix, GrokMuonAdEMA, GrokOrScale):
    def create_optimizer(
        ...
        r_min: float = 0.1,
        r_max: float = 5.0,
        projector_branch: str = "orscale",
    ):
        """
        Build optimizer V8:
          - 2D Matrices (LoRA / Linear) -> OrScale-LM / Muon (LR * lr_scale)
          - 1D Parameters (Norms / Bias) -> AdEMAMix (LR base)
        """
        if opt_type == "paged_adamw_8bit":
            return None

        _scale = lr_scale if opt_type in ("grokorscale", "grokmuonadema") else 1.0
        ...
        for _comp in ["encoder", "decoder", "projector", "vision_tower"]:
            ...
            if _comp == "projector" and projector_branch == "adema" and opt_type in ("grokorscale", "grokmuonadema"):
                if _p2d:
...
@app.cell
def _(format_mto_encoder_input, format_mto_target, load_dataset, random):
    def load_hf_samples(
        repo_id: str, config_name: str, split: str, n_samples: int, seed: int = 42
    ) -> list[dict]:
        """
        Download dataset dari HF Hub; sampling group-aware per chat_idx.
        """
    ...
    def text_sft_to_joint(samples, is_chat: bool, enable_mto: bool = True):
        """
        Baris teks (chat_sft / indoqa_sft) -> format joint vision-collator.
        Suntikkan MTO task prefix ke target Assistant.
        """
        rows = []
        task_category = "general_chat" if is_chat else "qa"
        if is_chat:
            chat_groups = {}
            for obj in samples:
            ...
            for chat_idx, turns in chat_groups.items():
                ...
                for turn in turns:
                    ...
                    if enable_mto:
                        tgt = format_mto_target(tgt, task_category)
                    rows.append({
                        "prompt_text": format_mto_encoder_input(turn["input"], "general_chat"),
                        "target_text": tgt,
                        "dataset_idx": -1,
                        "image_indices": [],
                        "images": [],
            ...
            for obj in samples:
                if not obj.get("input") or not obj.get("target"):
                    continue
                tgt = obj["target"].strip()
                if enable_mto:
                    tgt = format_mto_target(tgt, task_category)
                rows.append({
                    "prompt_text": format_mto_encoder_input(obj["input"], "qa"),
                    "target_text": tgt,
                    "dataset_idx": -1,
                    "image_indices": [],
                    "images": [],
    ...
    def text_orpo_to_joint(samples, enable_mto: bool = True):
        ...
        for obj in samples:
            ...
            if enable_mto:
                chosen_raw = format_mto_target(chosen_raw, "general_chat")
                rejected_raw = format_mto_target(rejected_raw, "general_chat")

            rows.append({
                "prompt_text": format_mto_encoder_input(obj["prompt"], "general_chat"),
...
def _(
    ...
    _labels = {
        "steering": "Phase 0.5 (DeVec SVD Task Vector Steering)",
        "cangkok": "Phase 1.5 (Vision Grafting)",
        "sft": "Phase 1 (JOINT SFT + MTO Prefixing)",
        "orpo": "Phase 2 (JOINT ORPO + TLPO)",
        "merge": "Final Merge",
        "done": "✅ SEMUA SELESAI",
    }
    print("=" * 70)
...
def _(mo):
    mo.md(r"""
    ---
    # 🧭 Phase 0.5 — DeVec SVD Task Vector Steering (arXiv:2512.22511)
    Menyuntikkan *vektor kemahiran instruksi murni* `Δ = SVD_Filter(W_Gemma3-IT − W_Gemma3-Base)`
    ke **DECODER** T5Gemma-2.

    - **DeVec SVD Subspace Filtering ($\tau=0.85$)**: Memisahkan interferensi unik donor dari coherent subspace.
    - **Layer-Wise Ramp-Up $\alpha$**: Early=0.05, Mid=0.25, Late=0.08 pada FFN dan Norm.
    - **Merged Attention Safety Guard**: $\alpha_{\text{QO}}=0, \alpha_{\text{KV}}=0, \alpha_{\text{QKNORM}}=0$ (Dipertahankan 100%).
    """)
    return
...
def _(
    ...
    # ---- DEVEC SVD SUBSPACE PURIFIER FUNCTION (arXiv:2512.22511) ----
    def devec_svd_purify(delta_weight: torch.Tensor, threshold: float = 0.85) -> torch.Tensor:
        """
        SVD subspace filtering (energy-based low-rank purification).

        Mempertahankan komponen singular dominan yang menyumbang >= `threshold`
        dari total energi (sum of squared singular values), lalu membuang
        komponen energi rendah (noise/interferensi) dari task vector.
        Ini interpretasi "DeVec SVD subspace filtering (tau=0.85)" untuk SATU
        task vector; dekomposisi shared/unique penuh dari paper memerlukan
        >= 2 task vector (di sini hanya ada satu delta W_IT - W_base).
        """
        ...
        try:
            ...
            # Jumlah komponen singular yang dibutuhkan untuk mencapai threshold energi
            ...
            # Fallback jika SVD gagal (mis. NaN/Inf) — kembalikan aslinya.
            return delta_weight

    def _lata_alignment_scale(delta: torch.Tensor, base: torch.Tensor) -> float:
        """
        LATA-style layer-wise alignment (arXiv:2502.20186).
        Skala berbasis |cos(delta, base)|:
          - |cos| tinggi -> delta searah instruction-following -> attenuate (skala < 1)
          - |cos| rendah -> delta task-specific -> amplify (skala > 1)
        """
    ...
    if _should_run:
        print("=" * 90)
        print("  [STEER] DeVec SVD Purified Task Vector Delta Steering (decoder T5Gemma-2)")
        print("=" * 90)
        print(f"  DeVec SVD Enabled: {ENABLE_DEVEC_STEERING} (tau={DEVEC_SVD_TAU})")
        ...
        _load_ok = False
        if not _token:
            raise RuntimeError("❌ [STEER] Error: HF_TOKEN belum diset.")
        else:
            gc.collect()
            try:
                print(f"\n[1/3] Loading Base T5Gemma-2: {BASE_T5_MODEL} (CPU)...")
                _t5 = _SteerSeq2Seq.from_pretrained(
                    BASE_T5_MODEL, dtype=torch.bfloat16, token=_token, trust_remote_code=True
                )
                print(f"[2/3] Loading Gemma 3 Base: {GEMMA_BASE_MODEL} (CPU)...")
                _g_base = _SteerCausal.from_pretrained(
                    GEMMA_BASE_MODEL, dtype=torch.bfloat16, token=_token, trust_remote_code=True
                )
                print(f"[3/3] Loading Gemma 3 IT: {GEMMA_IT_MODEL} (CPU)...")
                _g_it = _SteerCausal.from_pretrained(
                    GEMMA_IT_MODEL, dtype=torch.bfloat16, token=_token, trust_remote_code=True
                )
                _load_ok = True
            except Exception as _e_load:
                print(f"\n❌ [STEER] Gagal memuat model gated HF: {_e_load}")
                raise RuntimeError(f"Gagal memuat model donor steering: {_e_load}") from _e_load
            ...
            if _load_ok:
                ...
                def _steer(g_suf, t_suf, alpha, cat):
                    ...
                    if g_key and g_key in _gb_sd and t_key:
                        if _t5_sd[t_key].shape == _gi_sd[g_key].shape == _gb_sd[g_key].shape:
                            _delta = _gi_sd[g_key] - _gb_sd[g_key]
                            if ENABLE_DEVEC_STEERING and _delta.ndim == 2:
                                _delta = devec_svd_purify(_delta, threshold=DEVEC_SVD_TAU)
                            if ENABLE_LATA_ALIGNMENT and cat in ("ffn", "layernorm"):
                                alpha = alpha * _lata_alignment_scale(_delta, _gb_sd[g_key])
                            _t5_sd[t_key] += alpha * _delta
                ...
                for _l in range(_L):
                    ...
                    for _proj in ("gate_proj", "up_proj", "down_proj"):
                        _steer(f"layers.{_l}.mlp.{_proj}.weight",
                               ...
                               _curr_alpha_ffn, "ffn")
                    # Attention projections (Alpha = 0.0)
                    for _proj, _a in (
                ...
                if _mismatch:
                    print(f"  ⚠️ {len(_mismatch)} keys di-skip. Contoh: {_mismatch[:5]}")
                if _total == 0:
                    raise RuntimeError("[STEER] Tidak ada tensor yang tersuntik!")
                ...
                if STEERING_SMOKE_TEST:
                    print("\n  [SMOKE TEST] Generate 3 prompt singkat...")
                    ...
                    assert _smoke_tok is not None, "Tokenizer failed to load"
                ...
                # ---- 4. Save + patch tokenizer + upload ----
                _local = "/tmp/t5gemma2_steered_v8"
                ...
                _marker = upload_folder_atomic(
                    ...
                    _local,
                    STEERED_SUBFOLDER,
                    commit_message="Phase 0.5 DeVec SVD Purified Task Vector Steering",
                )
                write_local_provenance(_local, _marker)
                del _t5
                gc.collect()
                print("  ✅ [STEER] Selesai & terunggah.")
    return
...
def _(mo):
    mo.md(r"""
    ---
    # 🌱 Phase 1.5 — Precision Vision Grafting (SigLIP 400M)
    Mencangkokkan `vision_tower` (SigLIP 400M) + `multi_modal_projector` dari **Gemma 3 4B IT**
    ke checkpoint hasil Phase 0.5.
    """)
    return
    ...
    if _should_run:
        ...
        try:
            _model_tgt = _GraftSeq2Seq.from_pretrained(
                _tgt_id, dtype=torch.bfloat16, token=_token, trust_remote_code=True, **_tgt_kw
            )
        ...
        if _graft_ok and _model_src is not None and _model_tgt is not None:
            _src_params = {}
            ...
            _local_save = "/tmp/v8_vision_cangkok"
            ...
            _marker = upload_folder_atomic(
                ...
                _local_save,
                CANGKOK_SUBFOLDER,
                commit_message="Phase 1.5 Precision Vision Grafting: SigLIP 400M + projector dari Gemma 3 4B IT",
            )
        ...
        if not cangkok_ready:
            raise RuntimeError("❌ [CANGKOK] Gagal: Base model `cangkok/` belum tersedia di repo HF!")
        else:
...
def _(mo):
    mo.md(r"""
    ---
    # 🎓 Phase 1 — JOINT SFT + MTO Task Prefixing (Single-Stage Co-Training)
...
def _(
    ...
    if orpo_done or pipeline_stage in ("done", "merge"):
        ...
        if sft_done and not orpo_done:
            ...
            _cangkok_local = "/tmp/v8_vision_cangkok"
            ...
            for _cand_dir in (_cangkok_local, os.path.join(OUTPUT_DIR, CANGKOK_SUBFOLDER)):
                if os.path.isdir(_cand_dir) and not local_provenance_valid(_cand_dir, _marker_cangkok):
                    ...
                    print(f"  🧹 Cache lokal cangkok usang — dihapus: {_cand_dir}")
        ...
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        model, processor = FastVisionModel.from_pretrained(**_load_kwargs)
    ...
    class VisionORPOCollator:
        ...
        def __call__(self, batch):
            ...
            out = {
                ...
                "flaw": flaws, "rationale": rationales,
            }
            if pvals:
                out["pixel_values"] = torch.cat(pvals, dim=0) if pvals[0].ndim == 4 else torch.stack(pvals, dim=0)
            return out
...
def _(
    ...
    # ---- TLPO REGULARIZATION HELPER (arXiv:2604.26553) ----
    class TLPO_Regularizer:
        """
        Token-Level Policy Optimization penalty to eliminate Indonesian language confusion.
        Penalizes generation of tokens outside valid Indonesian vocabulary space.

        Objektif: PPO-style clipped surrogate di level token.
          ratio   = pi_topk / pi_old_topk  (pi_old = snapshot frozen dari step sebelumnya)
          loss    = -mean(min(ratio*A, clip(ratio, 1-eps, 1+eps)*A))
        A adalah advantage = reward - mean(reward) atas top-k kandidat token.
        """
        def __init__(self, suppress_ids: set, beta: float = 0.1, clip_eps: float = 0.2):
            self.suppress_ids = suppress_ids
            self.beta = beta
            self.clip_eps = clip_eps
            self._frozen_log_probs = None  # lagged/old policy snapshot (detached)
        ...
        def __call__(self, logits: torch.Tensor, labels: torch.Tensor):
            ...
            # Confusion mask: posisi di mana argmax jatuh pada token invalid/suppressed
            ...
            # PPO-style ratio vs policy snapshot (old policy) — trust region
            ...
            # Refresh snapshot policy untuk step berikutnya (detached, tanpa grad)
    ...
    class JointORPOTrainer(Seq2SeqTrainer):
        ...
        def _flaw_pair_weights(self, flaws, dtype, device):
            return torch.tensor(
                [self.FLAW_WEIGHTS.get(f, 1.0) for f in flaws],
                dtype=dtype, device=device,
            )

        def _repetition_token_weights(self, labels):
            """Token-level emphasis untuk flaw 'repetitive' (proksi heuristik).
            Up-weight posisi token yang ikut membentuk bigram berulang (muncul >= 2x)
            pada sequence rejected. B kecil -> loop per baris demi kejelasan & memori."""
            ...
            for b, row in enumerate(ids):
                ...
                for i, bg in enumerate(bigrams):
                    if counts[bg] >= 2:
                        w[b, i] = 1.0 + self.flaw_lambda
                        w[b, i + 1] = 1.0 + self.flaw_lambda
            return w.to(labels.device)

        def get_batch_logps(self, logits, labels, average_log_prob=True, token_weights=None):
            labels = labels.clone()
            mask = labels != -100
            labels[labels == -100] = 0
            lps = torch.gather(logits.log_softmax(-1), dim=2, index=labels.unsqueeze(2)).squeeze(2)
            if token_weights is not None:
                mask = mask * token_weights.to(lps.dtype)
            if average_log_prob:
        ...
        def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None, **kwargs):
            ...
            flaws = inputs.pop("flaw", None) or []
            inputs.pop("rationale", None)
            ...
            r_token_weights = None
            if self.enable_flaw_loss and any(f == "repetitive" for f in flaws):
                r_token_weights = self._repetition_token_weights(rl)

            clp = self.get_batch_logps(co.logits, cl)
            rlp = self.get_batch_logps(ro.logits, rl, token_weights=r_token_weights)

            # Log-odds stability formula
            ...
            per_pair = -F.logsigmoid(clo - rlo)

            # Option 3 (flaw-aware): bobot per-pair dari label `flaw`, bukan TLPO paper.
            if self.enable_flaw_loss and flaws:
                pair_w = self._flaw_pair_weights(flaws, per_pair.dtype, per_pair.device)
                or_loss = (pair_w * per_pair).sum() / pair_w.sum().clamp(min=1)
            else:
                or_loss = per_pair.mean()

            # TLPO regularizer (heuristik suppress-ids — bukan paper TLPO 2604.26553)
            tlpo_loss = self.tlpo_regularizer(co.logits, cl) if self.enable_tlpo else 0.0

            loss = co.loss + (self.beta * or_loss) + tlpo_loss
            ...
            if bertscore_metric is not None:
                if _bs_state["available"] is None:
                    try:
                        ...
                        print(f"ℹ️ BERTScore dinonaktifkan: {_e_bs}")
...
@app.cell
def _(torch):
    def evaluate_culturetalk_id(
        model,
        tokenizer,
        dataset_id: str = "indobenchmark/culturetalk-id",
        split: str = "test",
        max_samples: int = 512,
        seed: int = 42,
    ):
        """
        Evaluasi CultureTalk-ID (arXiv:2607.21016) — benchmark dialog budaya Indonesia
        (11 bahasa lokal, 13 topik, 3 task).

        Baru Task 1 (dialogue-based multiple-choice cultural commonsense reasoning)
        yang diimplementasikan (skor = pilih opsi dengan teacher-forcing loss terkecil).
        Task 2 (culturally faithful machine translation) & Task 3 (language steering)
        masih TODO. Return dict metrics, atau {} bila dataset tidak tersedia.
        """
        metrics = {}
        try:
            from datasets import load_dataset as _load_culture
            ds = _load_culture(dataset_id, split=split)
        except Exception as e:
            print(f"[CultureTalk-ID] dataset tidak tersedia, skip: {e}")
            return metrics
        ...
        if max_samples and len(rows) > max_samples:
            ...
            _random.seed(seed)
            rows = _random.sample(rows, max_samples)
        ...
        correct = total = 0
        for row in rows:
            ctx = row.get("context") or row.get("dialogue") or ""
            question = row.get("question") or row.get("query") or ""
            options = row.get("options") or row.get("choices")
            answer = row.get("answer") if row.get("answer") is not None else row.get("answer_idx")
            ...
            for opt in options:
                ...
                with torch.no_grad():
                    ...
                    opt_scores.append(float(out.loss.item()))
            if not opt_scores:
                continue
            pred = min(range(len(opt_scores)), key=lambda i: opt_scores[i])
        ...
        metrics["culturetalk_mc_accuracy"] = (correct / total) if total else 0.0
...
def _(
    ...
    class VisionTrainingPlotCallback(TrainerCallback):
        ...
        def plot_chart(self) -> None:
            ...
            ax1.set_ylabel("Loss")
            ax1.set_title("V8 Training & Evaluation Loss Curve")
            ...
            ax3.set_title("Multimodal Quality Metrics")
            ...
            ax4.set_title("Text-Only Quality Metrics")
...
def _(
    ...
    if _should_run:
        ...
        _optimizer = create_optimizer(
            ...
            momentum=ORSCALE_MOMENTUM,
            ns_steps=ORSCALE_NS_STEPS,
            nesterov=ORSCALE_NESTEROV,
            max_grad_norm=ORSCALE_MAX_GRAD_NORM,
            lr_scale=SFT_ORSCALE_SCALE,
            r_min=ORSCALE_R_MIN,
            r_max=ORSCALE_R_MAX,
        ...
        if sft_resume:
            try:
                ...
                raise RuntimeError(
                    "❌ [JOINT-SFT] Resume ditandai tersedia di HF tapi gagal di-download — pipeline dihentikan."
                ) from e

        print("\n🚀 [JOINT-SFT] Starting JOINT SFT training V8 (vision + teks + MTO + OrScale)...")
...
def _(mo):
    mo.md(r"""
    ---
    # 🎯 Phase 2 — JOINT ORPO + TLPO Regularization (arXiv:2604.26553)
    - **Single Preference Alignment Loop**: `vision_orpo` + `chat_orpo` (100% data).
    - **TLPO Loss Function**: Menghukum token model yang keluar dari distribusi bahasa Indonesia ($\lambda=0.05$).
    - **Split Forward (Encoder $\rightarrow$ Decoder)**: Menghemat 40% VRAM.
    - **Label Smoothing = 0.0 (Mandatory)**: Menjaga integritas probabilitas odds-ratio.
    """)
    return
...
def _(
    ...
    flaw_list = raw_orpo_dataset["flaw"]
    rationale_list = raw_orpo_dataset["rationale"]
    ...
    for _idx_orpo in range(len(prompts_list)):
        ...
        if ENABLE_MTO_PREFIX:
            chosen_raw = format_mto_target(chosen_raw, "vision")
            rejected_raw = format_mto_target(rejected_raw, "vision")
        ...
        vision_orpo_rows.append({
            ...
            "_modality": "vision",
            "flaw": flaw_list[_idx_orpo],
            "rationale": rationale_list[_idx_orpo],
        })
    ...
    if joint_eval_multimodal is not None:
        for _r_mm in joint_eval_multimodal:
            ...
            _orpo_eval_mm_rows.append({
                ...
                "rejected_text": _r_mm["target_text"],
                "images": _imgs_o,
                "dataset_idx": -1,
                "_modality": "vision",
                "flaw": "",
    ...
    orpo_eval_mm = Dataset.from_list(_orpo_eval_mm_rows, on_mixed_types="use_json") if _orpo_eval_mm_rows else None
    ...
    orpo_eval_text = Dataset.from_list(_orpo_eval_text_rows, on_mixed_types="use_json") if _orpo_eval_text_rows else None

    joint_orpo_train_dataset = Dataset.from_list(joint_orpo_rows, on_mixed_types="use_json")
    ...
    print(f"\n  ✅ JOINT ORPO train V8: {len(joint_orpo_train_dataset)} | eval (set SFT): {list(joint_orpo_eval_datasets.keys())}")
    return (
        joint_orpo_eval_datasets,
...
def _(
    ...
    if _should_run:
        ...
        print(f"[JOINT-ORPO] Output dir V8: {joint_orpo_output_dir}")
        print(f"[JOINT-ORPO] Train: {len(joint_orpo_train_dataset)} | Eval sets: {list(joint_orpo_eval_datasets.keys())} | beta={ORPO_BETA} | tlpo_lambda={TLPO_LAMBDA}")
        ...
        joint_orpo_trainer = JointORPOTrainer(
            ...
            args=Seq2SeqTrainingArguments(
                ...
                bf16=BF16,
                optim=_optim_str_o,
                label_smoothing_factor=ORPO_LABEL_SMOOTHING_FACTOR,
                gradient_checkpointing=True,
                eval_strategy="steps",
        ...
        if orpo_resume:
            try:
                ...
                raise RuntimeError(
                    "❌ [JOINT-ORPO] Resume ditandai tersedia di HF tapi gagal di-download — pipeline dihentikan."
                ) from e

        print("\n🚀 [JOINT-ORPO] Starting JOINT ORPO training V8 (with TLPO regularizer)...")
    ...
    if not _should_merge:
        print(
            ...
            + (
                ...
                if final_done
                else "adapter ORPO/SFT belum ada & model tidak dimuat."
            )
        )
    else:
...
def _(UNIFIED_HF_REPO, mo):
    mo.md(f"""
    ---
    ### 💻 Deployment & Inference (Unified Repo V8)
    Repo: **`{UNIFIED_HF_REPO}`** (PUBLIC)

    ```
    steered/            → checkpoint Phase 0.5 (DeVec SVD Task Vector Steering)
    cangkok/            → checkpoint Phase 1.5 (SigLIP+projector graft, base training)
    joint/sft/          → checkpoints + final_adapter SFT joint V8
    joint/orpo/         → checkpoints + final_adapter ORPO joint V8
...
def _(mo):
    mo.md(r"""
    ---
    ## 🧪 Evaluasi Pasca-Training V8 (berjalan hanya jika model masih di memori)
    """)
    return
...
def _(
    ...
    if model is not None:
        ...
        print("\n" + "=" * 70)
        print("[EVAL V8] TEST 2: Pemeliharaan chat teks (20 kueri pertama dari validation)")
        print("=" * 70)
        ...
        try:
            ...
            print(f"[EVAL V8] {len(eval_samples)} sampel validasi teks dimuat.")
        ...
        print("⏭️ [EVAL V8] Model tidak di memori (training belum jalan di sesi ini) — eval dilewati.")
    return
