...
OUTPUT_TRAIN = DATA_DIR / "chat_train.jsonl"
OUTPUT_VAL = DATA_DIR / "chat_val.jsonl"
...
from scripts.dataset.flatten_conversations_jsonl_to_sft import (
...
def main():
    if not INPUT_EDITED.exists():
        ...
        print("Please run `scripts/dataset/edit_prefix_tasks.py` first to generate it.")
        sys.exit(1)
    ...
    # Validation: Exactly 100 conversations
    val_size = min(100, total_conversations)
    val_convs = conversations[:val_size]
    train_convs = conversations[val_size:]

    print(f"[INFO] Splitting dataset: {len(train_convs)} train threads, {len(val_convs)} validation threads.")

    # 4. Flatten and calculate tokens
    ...
    def process_and_write(convs, output_path):
        ...
        with output_path.open("w", encoding="utf-8") as out_f:
            for item in convs:
                ...
                # Convert nested conversations to SFT rows
                sft_rows = conversations_to_sft_rows(messages, fallback_sys, chat_idx)
                
                for row in sft_rows:
                    # Calculate exact tokens using local tokenizer
    ...
    train_rows_count = process_and_write(train_convs, OUTPUT_TRAIN)
    val_rows_count = process_and_write(val_convs, OUTPUT_VAL)

    print("\n==================================================")
    print("SUCCESSFULLY MERGED & SPLIT DATASETS")
    print("==================================================")
    print(f"Validation Set: {len(val_convs)} conversations -> {val_rows_count} SFT turns")
    print(f"Training Set:   {len(train_convs)} conversations -> {train_rows_count} SFT turns")
    print(f"Train File:     {OUTPUT_TRAIN}")
    print(f"Val File:       {OUTPUT_VAL}")
    print("==================================================")
