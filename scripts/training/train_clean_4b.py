"""
SFT Training: T5Gemma-2 4B-4B Clean SFT (10% Trainable Parameters)
===================================================================
Melatih model 4B-4B secara penuh pada seluruh dataset (31,299 baris).
LoRA rank r=128 melatih ~755 juta parameter (~10.6% trainable).
Logit Masking digunakan untuk menyaring unused & vision tokens.
"""
...
OUTPUT_DIR = "results/t5gemma2-4b-clean-sft"

CHAT_TRAIN_FILE   = "data/chat_train.jsonl"
INDOQA_TRAIN_FILE = "data/indoqa_train.jsonl"

# Ditentukan berdasarkan analisis distribusi dataset (meng-cover >99% data)
MAX_SOURCE_LENGTH = 1536
MAX_TARGET_LENGTH = 512

# 0 berarti memakai seluruh data (total 31,299 sampel)
SAMPLE_CHAT   = 0   
SAMPLE_INDOQA = 0   

# Token IDs yang harus di-suppress
SUPPRESS_BLOCK1 = list(range(6, 105))             # <unused0>–<unused98>
SUPPRESS_BLOCK2 = list(range(256002, 262144))     # <unused100>–<unused6241>
SUPPRESS_VISION = [255999, 256000, 256001]        # <img>, <end_of_image>, <image_soft_token>
...
class TrainingPlotCallback(TrainerCallback):
    ...
    def _save_chart(self) -> None:
        ...
        ax.set_xlabel("Steps")
        ax.set_ylabel("Loss")
        ax.set_title("Training Loss Curve — T5Gemma2-4B Clean SFT")
        ...
        if len(self.losses) >= 10:
            ...
            ax.plot(self.steps, ma, color="#E74C3C", linewidth=2, label="MA-10", alpha=0.8)
            ax.legend()
        plt.tight_layout()
...
def main() -> None:
    ...
    print(f"Total training rows (Clean SFT): {len(final_rows)}")
    ...
    # LoRA Config: r=128, lora_alpha=256 untuk melatih ~10.6% parameter
    ...
    sample_callback = SampleGenerationCallback(
        ...
        eval_every_n_steps=200,  # Evaluasi behavior setiap 200 steps
    )
    ...
    # Training args dengan batch_size=2, grad_accum=4 (effective batch = 8)
    training_args = Seq2SeqTrainingArguments(
        output_dir=OUTPUT_DIR,
        per_device_train_batch_size=2,
        gradient_accumulation_steps=4,
        learning_rate=5e-5,
        num_train_epochs=3,  # 3 epochs sudah cukup konvergen untuk dataset penuh
        warmup_steps=100,
        weight_decay=0.01,
    ...
    trainer = Seq2SeqTrainer(
        ...
        callbacks=[
            plot_callback, 
            sample_callback, 
            EarlyStoppingCallback(early_stopping_patience=10) # Stop jika eval_loss memburuk terus selama 10 eval (2000 steps)
        ],
    )
