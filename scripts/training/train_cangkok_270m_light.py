"""
SFT Training: T5Gemma-2 270M Cangkok (Light Version)
======================================================
Melatih model 270M dengan 1000 data percakapan terpilih.
Epoch singkat (2 epochs) dengan callback monitoring behavior real-time setiap 50 steps.
"""
...
OUTPUT_DIR = "results/t5gemma2-270m-light-sft"
...
SAMPLE_CHAT   = 70   # 70 sampel percakapan
SAMPLE_INDOQA = 30   # 30 sampel QA
...
class TrainingPlotCallback(TrainerCallback):
    ...
    def _save_chart(self) -> None:
        ...
        ax.set_xlabel("Steps")
        ax.set_ylabel("Loss")
        ax.set_title("Training Loss Curve — T5Gemma2-270M Light SFT")
...
def main() -> None:
    ...
    print(f"Total training rows (Light SFT): {len(final_rows)}")
    ...
    sample_callback = SampleGenerationCallback(
        ...
        eval_every_n_steps=10,  # Evaluasi behavior setiap 10 steps
    )
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        learning_rate=5e-5,
        num_train_epochs=10,  # Ditingkatkan agar model benar-benar konvergen pada subset kecil
        warmup_steps=50,
        weight_decay=0.01,
