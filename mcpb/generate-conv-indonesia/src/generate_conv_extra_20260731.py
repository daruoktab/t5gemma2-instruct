...
try:
    ...
    # Bundle MCPB tidak memakai agent (Claude yang menulis) — import opsional.
    Agent: Any = None
            ...
            if m.role == "user":
                ...
                for p in m.prefixes:
                    ...
                    if p and p not in cleaned:
                        cleaned.append(p)
                if not cleaned:
                    # JANGAN fallback diam-diam ke <unused4> — tolak supaya model memperbaiki.
                    raise ValueError(
...
class PureHuggingFaceDataLoader:
    ...
    def _load_cvqa_id_metadata() -> Optional[datasets.Dataset]:
        ...
        try:
            ...
            if not file_urls:
                print("      ⚠️ CVQA: daftar parquet kosong, dilewati.", file=sys.stderr)
                return None
            ...
            if id_table.num_rows == 0:
                print("      ⚠️ CVQA: tidak ada subset Indonesia ditemukan, dilewati.", file=sys.stderr)
                return None
            ...
            print(f"      ⚠️ CVQA gagal dimuat (metadata-only): {e}", file=sys.stderr)
            return None
    ...
    def load_all(self):
        ...
        # 1. FineWeb-Edu-25K (skip di quick — muat via include_heavy)
        try:
            if self.quick:
                print("  --> ⏭️ FineWeb-Edu-25K dilewati (quick)")
            else:
        ...
        # 2. OpenWebText-Indonesia-10k (skip di quick)
        try:
            if self.quick:
                print("  --> ⏭️ OpenWebText-Indonesia-10k dilewati (quick)")
            else:
        ...
        # 3. Indonesian Simple Summaries (skip di quick)
        try:
            if self.quick:
                print("  --> ⏭️ Indonesian Simple Summaries dilewati (quick)")
            else:
        ...
        # 7. QQPR-Triplets-ID (skip di quick)
        try:
            if self.quick:
                print("  --> ⏭️ QQPR-Triplets-ID dilewati (quick)")
            else:
        ...
        # 8. Indonesian Wikipedia (skip di quick)
        try:
            if self.quick:
                print("  --> ⏭️ Indonesian Wikipedia dilewati (quick)")
            else:
        ...
        # 11. CVQA subset Indonesia (BARU: metadata-only VQA budaya) — skip di quick (partial-read lambat)
        try:
            if self.quick:
                print("  --> ⏭️ CVQA-Indonesia dilewati (quick)")
            else:
        ...
        # 15. IndoRad-VQA (BARU: VQA radiologi Bahasa Indonesia, train+test) — skip di quick
        try:
            if self.quick:
                print("  --> ⏭️ IndoRad-VQA dilewati (quick)")
            else:
...
4. GAYA TURN USER (PENTING): turn user harus ditulis seperti MANUSIA biasa yang bertanya, BUKAN robot/AI —
   variasi gaya & panjang kalimat (santai/formal, pendek/panjang), bahasa sehari-hari natural,
   jangan memulai semua turn dengan pola yang sama ("Halo", "Tolong", "Bisakah kamu..."),
   jangan menulis dari sudut pandang AI di turn user (mis. "Saya akan membantu Anda...", "Sebagai asisten..."),
   dan pertanyaan follow-up harus nyambung natural dari jawaban sebelumnya seperti percakapan sungguhan —
   bukan daftar pertanyaan berurutan yang terasa scripted.
5. GROUNDING: SEBELUM menulis, identifikasi 2-3 FAKTA KUNCI dari teks mentah asli yang akan menjadi
   dasar percakapan, lalu pastikan seluruh turn konsisten dengan fakta tersebut — JANGAN menambah
   fakta di luar konteks.
6. DISTRIBUSI PREFIX (lintas percakapan): usahakan SEMUA token <unused1..6> terpakai SEIMBANG di
   seluruh dataset — jangan hanya <unused4>. Variasikan antar percakapan sesuai task isi pesan
   (mis. percakapan 1: <unused1>/<unused4>, percakapan 2: <unused5>/<unused6>).
"""
    return prompt.strip()
...
1. Prefix turn assistant TEPAT & sesuai isi pesan. Token: <unused1> SUMMARIZE, <unused2> TRANSLATE,
   ...
   Boleh 1-3 token UNIK tanpa spasi/duplikat jika pesan multi-task (mis. <unused1><unused2>).
   Variasikan lintas percakapan — jangan semua turn memakai token yang sama jika task-nya berbeda.
...
7. Turn USER seperti manusia biasa (variasi gaya/panjang kalimat, tanpa pola repetitif seperti "Halo"/"Tolong" di tiap turn, bukan sudut pandang AI seperti "Saya akan membantu Anda...").
...
async def generate_single_conversation_pydantic(
    ...
    for idx, msg in enumerate(final_convs):
        ...
        # Bersihkan token prefix yang bocor ke konten (model sering meniru
        # format target dengan menulis '<unused4> <unused4>' di dalam konten)
        # — strip di SELURUH konten; token valid hanya di field `prefixes`.
        content_clean = re.sub(r"<unused[1-6]>", "", content_clean)
        content_clean = re.sub(r"\s{2,}", " ", content_clean).strip()
...
async def main_async():
    ...
    parser.add_argument("--output", type=Path, default=DATA_DIR / "synthetic" / "generated_conv_agent.jsonl", help="Path file output JSONL")
    ...
    agent = Agent(
        ...
        instructions="Kamu adalah generator percakapan sintetis terstruktur yang mematuhi skema Pydantic. Kamu HANYA mengembalikan token prefix murni seperti ['<unused1>'] tanpa pernah menyertakan teks kata 'SUMMARIZE' atau 'NER'. Tulis turn USER seperti manusia biasa yang bertanya (variasi gaya, bukan robot/AI, bukan sudut pandang asisten) dan turn ASSISTANT sesuai persona Gemma."
    )
